...
const BUILDKIT_CONTAINER_NAME =
	process.env.NOUVA_AGENT_BUILDKIT_CONTAINER || "nouva-buildkitd";
...
const TRAEFIK_CONTAINER_NAME =
	process.env.NOUVA_AGENT_TRAEFIK_CONTAINER || "nouva-traefik";
const BUILDKIT_ADDRESS =
	process.env.NOUVA_AGENT_BUILDKIT_ADDR || "tcp://127.0.0.1:1234";
const DEFAULT_BUILDKIT_PORT = 1234;
const BACKUP_HELPER_IMAGE =
	process.env.NOUVA_BACKUP_HELPER_IMAGE ||
	"ghcr.io/nouvacloud/backup-helper:latest";
...
function getInheritedNouvaEnvKeys(
...
function resolveHydratedHelperSpec(payload: {
	...
	if (!payload.imageUrl || !payload.dataPath || !payload.envVars) {
		throw new Error(
			"Backup helper payload is missing hydrated executor fields",
		);
	}
...
async function collectValidationSnapshot(
	...
	checks.push(
		buildCheck(
			...
			osSupported
				...
				: "Nouva currently supports Ubuntu 24.04 only",
	...
	checks.push(
		buildCheck(
			...
			archSupported
				? "x86_64 detected"
				: "Nouva currently supports x86_64 only",
			hostArch,
		),
	);
	...
	try {
		const version = await docker.request<{ Version?: string }>(
			"GET",
			"/version",
		);
	...
	try {
		const ipForward = (
			await readFile("/hostfs/proc/sys/net/ipv4/ip_forward", "utf8")
		).trim();
		checks.push(
			buildCheck(
				"ip-forward",
		...
		checks.push(
			buildCheck(
				...
				error instanceof Error
					? error.message
					: "Unable to read IP forwarding state",
			),
		);
	}
	...
	try {
		...
		try {
			...
			isStub =
				/^nameserver\s+127\.0\.0\.53$/m.test(content) &&
		...
		checks.push(
			buildCheck(
				...
				error instanceof Error
					? error.message
					: "Unable to inspect DNS configuration",
			),
		);
	}
	...
	try {
		const maxWatches = parseInt(
			(
				await readFile("/hostfs/proc/sys/fs/inotify/max_user_watches", "utf8")
			).trim(),
		);
		checks.push(
			buildCheck(
...
async function registerAgent(docker: DockerApiClient): Promise<{
	...
	const payload = await apiRequest<AgentRegistrationResponse>(
		"/api/agent/register",
		{
			method: "POST",
			body: {
...
function buildLabels(input: {
	...
	return {
		...
		...(input.deploymentId
			? { "nouva.deployment.id": input.deploymentId }
			: {}),
		...(input.serviceVariant
			? { "nouva.service.variant": input.serviceVariant }
			: {}),
	};
}
...
async function waitForBuildkitAvailability(
	address: string,
	timeoutMs = 15_000,
): Promise<void> {
	...
	throw new Error(
		`BuildKit did not become ready at ${address} within ${timeoutMs}ms`,
	);
}
...
type ManagedContainerRecord = Awaited<
	ReturnType<DockerApiClient["listManagedContainers"]>
>[number];
...
function parsePgBackrestInfo(raw: string): PgBackrestInfoBackup[] {
	...
	return backups
		...
		.filter(
			(entry: PgBackrestInfoBackup | null): entry is PgBackrestInfoBackup =>
				entry !== null,
		);
}
...
function selectCurrentPgBackrestEntry(
	...
	const byAnnotation = entries.find(
		(entry) => entry.annotationBackupId === backupId,
	);
	if (byAnnotation) {
		return byAnnotation;
	}
...
async function runTaskContainer(
	...
	try {
		...
		if (statusCode !== 0) {
			throw new Error(
				logs.trim() || `Task container ${options.name} failed (${statusCode})`,
			);
		}
...
async function ensureBaseRuntime(
	...
	const existingBuildkit = await docker.inspectContainer(
...
export function buildAppContainerSpec(
	...
	return {
		...
		spec: {
			name: containerName,
			image: payload.imageUrl,
			env: Object.entries(payload.envVars).map(
				([key, value]) => `${key}=${value}`,
			),
			labels: buildLabels({
				kind: "app",
...
export async function deployAppImage(
	...
	const { containerName, appPort, spec } = buildAppContainerSpec(
		config,
		payload,
	);
	if (payload.volume) {
	...
	await docker
		.connectNetwork(projectNetwork, containerId)
		.catch((err: Error) => {
			console.error(
...
function getManagedVolumeName(
	payload: DatabaseProvisionPayload | DeleteVolumePayload,
): string {
...
export function buildDatabaseContainerSpec(payload: DatabaseProvisionPayload): {
	...
	return {
		...
		spec: {
			...
			env: Object.entries(resolved.envVars).map(
				([key, value]) => `${key}=${value}`,
			),
			cmd:
				resolved.containerArgs.length > 0 ? resolved.containerArgs : undefined,
			labels: buildLabels({
				kind: "database",
				projectId: payload.projectId,
				serviceId: payload.serviceId,
...
async function handleCreateArchiveBackup(
	...
	const sizeBytes = Number.parseInt(
		extractPrefixedLogLine(logs, "NOUVA_SIZE_BYTES:") ?? "",
		10,
	);
	return {
...
async function handleExpireVolumeBackupRepository(
	...
	const { logs } = await runTaskContainer(docker, {
		...
		env: Object.entries(toRecord(payload.envVars)).map(
			([key, value]) => `${key}=${value}`,
		),
		cmd: [
			"sh",
...
async function handleRestart(
	...
	return {
		runtimeMetadata: {
			...
			containerName:
				payload.containerName ?? payload.runtimeMetadata?.containerName ?? null,
		},
	};
}
...
async function handleRemove(
	...
	const identifier =
		runtimeMetadata?.containerId ?? runtimeMetadata?.containerName;
	if (identifier) {
		await docker.removeContainer(identifier, true);
	}
...
async function handleSyncRouting(
	...
	return {
		runtimeMetadata: {
			...runtimeMetadata,
			configVersion:
				typeof runtimeMetadata.configVersion === "number"
					? runtimeMetadata.configVersion + 1
					: 1,
		},
	};
}
...
async function handleUpdateAgent(
	...
	const { updaterEnv, envInheritFlags } = buildUpdateAgentRuntimeEnv(
		process.env,
		imageRef,
	);
...
async function processWorkItem(
	...
	console.log(
		`[nouva-agent] processing work ${workItem.id} (${workItem.kind})`,
	);
	...
	try {
		switch (workItem.kind) {
			...
			case "delete_service":
				...
				break;
			case "delete_volume":
				result = await handleDeleteVolume(
					docker,
		...
		workError =
			err instanceof Error ? err : new Error("Unknown agent work failure");
	}
	...
	if (workError) {
		try {
			...
			console.error(
				`[nouva-agent] failed to report failure for work ${workItem.id}:`,
				reportErr,
			);
		}
		return;
	...
	try {
		...
		console.log(
			`[nouva-agent] work ${workItem.id} (${workItem.kind}) completed`,
		);
	} catch (reportErr) {
		// Work succeeded locally. Don't call /fail — let lease expire so the item can be retried.
		console.error(
			`[nouva-agent] work ${workItem.id} succeeded but /complete failed:`,
			reportErr,
		);
	}
}
...
async function syncRuntimeLogs(
	...
	const containers = (await docker.listManagedContainers())
		...
		.filter(
			(container): container is ManagedRuntimeContainer => container !== null,
		);
	...
	for (const container of containers) {
		try {
			...
			const entries = await docker.containerLogEntries(container.id, {
				...
				since:
					cursor.lastTimestampMs > 0
						? new Date(Math.max(0, cursor.lastTimestampMs - 1000))
						: null,
			});
	...
	const response = await apiRequest<AgentRuntimeLogsResponse>(
		"/api/agent/logs/runtime",
		{
			method: "POST",
			token: credentials.agentToken,
			body: {
...
async function main() {
	...
	if (config.postgresObservabilityIntervalSeconds > 0) {
		setInterval(() => {
			...
			syncPostgresObservability(docker, credentials!)
				.catch((error) => {
					console.error(
						"[nouva-agent] postgres observability sync failed",
						error,
					);
				})
				.finally(() => {
	...
	setInterval(async () => {
		...
		workLoopActive = true;
		try {
			const leased = await apiRequest<AgentLeaseResponse>(
				"/api/agent/work/lease",
				{
					method: "POST",
					token: credentials!.agentToken,
					body: {
