...
export interface PlatformBackupDestination
	extends PlatformBackupDestinationMetadata {
	accessKeyId: string;
...
export interface CreateVolumeBackupPayload
	extends QueuedVolumeBackupPayloadBase {
	backupId: string;
	kind: string;
...
export interface DeleteVolumeBackupPayload
	extends QueuedVolumeBackupPayloadBase {
	backupId: string;
	engine: "pgbackrest" | "snapshot";
	destination: PlatformBackupDestination;
...
export function getAgentRuntimeConfig(): AgentRuntimeConfig {
	const registryPort = Number.parseInt(
		process.env.NOUVA_AGENT_LOCAL_REGISTRY_PORT ?? "5000",
		10,
	);
	...
	return {
		...
		pollIntervalSeconds: Number.parseInt(
			process.env.NOUVA_AGENT_POLL_INTERVAL_SECONDS ??
				String(DEFAULT_AGENT_POLL_INTERVAL_SECONDS),
			10,
		),
		leaseTtlSeconds: Number.parseInt(
			process.env.NOUVA_AGENT_LEASE_TTL_SECONDS ??
				String(DEFAULT_AGENT_LEASE_TTL_SECONDS),
			10,
		),
		...
		localRegistryHost:
			process.env.NOUVA_AGENT_LOCAL_REGISTRY_HOST ?? "127.0.0.1",
		localRegistryPort: Number.isFinite(registryPort) ? registryPort : 5000,
		localTraefikNetwork:
			process.env.NOUVA_AGENT_INGRESS_NETWORK ?? "nouva-ingress",
	};
}
...
export function negotiateDockerApiVersion(
...
function parseMeminfoValue(content: string, key: string): number | null {
	...
	const value = Number.parseInt(
		line.replace(`${key}:`, "").trim().split(/\s+/)[0] ?? "",
		10,
	);
